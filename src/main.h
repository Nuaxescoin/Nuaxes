...
static const int LAST_POW_BLOCK = 60001;
...
public:
    ...
    {
        // Nuaxes: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CTxDB& txdb, uint64_t& nCoinAge) const;  // Nuaxes: get transaction coin age
...
public:
    ...
    // Nuaxes: block signature - signed by one of the coin base txout[N]'s owner
    ...
    // Nuaxes: two types of block: proof-of-work or proof-of-stake
    ...
    // Nuaxes: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64_t& nCoinAge) const; // Nuaxes: calculate total coin age spent in block
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    uint256 nChainTrust; // Nuaxes: trust score of block chain
    int nHeight;

    int64_t nMint;
    int64_t nMoneySupply;

    unsigned int nFlags;  // Nuaxes: block index flags
    enum
    {
